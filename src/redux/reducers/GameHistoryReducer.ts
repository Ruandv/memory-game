...
export default function GameHistoryReducer (state: IGameState = {} as IGameState, action: ActionType) {
  ...
  const deviceUniqueId = storageService.getValue('UniqueId', true);
  // The reducer normally looks at the action type field to decide what happens
  let result: any;
  const gameItem = action.payload as IGameItem;
  const idx = state.history.findIndex(x => x.id === gameItem.id);
  switch (action.type) {
